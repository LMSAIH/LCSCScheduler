...
def get_availabilities(role: Optional[str] = Query(None, description="Role to filter by"), supabase: Client = Depends(get_supabase), admin_user = Depends(check_admin) ) -> List[Availability]:
    try:
        ...
        if role:
            ...
            events_query = supabase.table("events").select("*, profiles!inner(*, user_roles!inner(*))").filter("profiles.user_roles.roles", "cs", f'{{"{role}"}}').or_(f"start_date.is.null, and(start_date.gte.'{prev_sunday}', start_date.lte.'{next_saturday}')").execute() # TODO
        else:
            profiles_query = supabase.table("profiles").select("*").eq("verified", True).execute()
            events_query = supabase.table("events").select("*, profiles(*, user_roles(*))").or_(f"start_date.is.null, and(start_date.gte.'{prev_sunday}', start_date.lte.'{next_saturday}')").execute() # TODO
        ...
        for event in events:
            ...
            while slot_time < event_end:
                slot_key = slot_time
                slot_event_counts[slot_key] = slot_event_counts.get(slot_key, 0) + 1
        ...
        for day in range(7):
            ...
            for hour in range(7, 20):
                ...
                events_overlapping = slot_event_counts.get(slot_start, 0)
